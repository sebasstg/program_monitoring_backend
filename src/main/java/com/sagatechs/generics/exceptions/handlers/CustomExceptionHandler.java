...
import com.sagatechs.generics.exceptions.BusinessException;
...
//port static com.github.adminfaces.template.util.Assert.has;
...
public class CustomExceptionHandler extends ExceptionHandlerWrapper {
	...
	private void goToErrorPage(FacesContext context, Throwable e) {
		...
		String userIp = request.getHeader("x-forwarded-for") != null
		...
		try {
			...
			logger.log(Level.SEVERE, "Could not redirect user to error page: "
					+ context.getExternalContext().getRequestContextPath() + errorPage, ex);
		}
	}
	...
	/**
	 * Older versions of PrimeFaces (6.1) doesn't have new PrimeFaces.current() so
	 * we must use RequestContext
	 *
	 * @return
	 */
